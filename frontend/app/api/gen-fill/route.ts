...
export async function POST(request: NextRequest) {
  try {
    const { imgUrl, aspect, width, height } = await request.json();
    ...
    //
    if (imgUrl.includes("e_gen_fill")) {
      url = imgUrl.replace("/e_gen_fill", "");
    }

    if (imgUrl.includes("b_gen_fill")) {
      url = imgUrl.replace(",b_gen_fill", "");
    }

    const parts = url.split("/upload/");
    const getFillUrl = `${parts[0]}/upload/ar_${aspect},b_gen_fill,c_pad,w_${width},h_${height}/${parts[1]}`;
    ...
    for (let attempts = 0; attempts < maxAttempts; attempts++) {
      isProcessed = (await checkingImageProcessing(getFillUrl))!;

      if (isProcessed) {
        return NextResponse.json(
          { getFillUrl, success: true },
          { status: 200 }
        );
      }
      ...
      console.log(getFillUrl);

      return NextResponse.json({ getFillUrl, success: true }, { status: 200 });
    }
  } catch (error) {
    console.log("Error generative fill", error);
    return NextResponse.json(
      { error: "generative fill failed" },
      { status: 500 }
    );
  }
}
